...
TEST_CASE("1.FollowsTExtractor.Extract FollowsT basic conditions") {
  ...
  SECTION("Single level statement list") {
    ...
    // Follows*(1, _)
    std::list<Statement*> expected_follows_1 = std::list<Statement*>{assign_3_, read_x_, print_y_};
    ...
    // Follows*(2, _)
    std::list<Statement*> expected_follows_2 = std::list<Statement*>{read_x_, print_y_};
    ...
    // Follows*(3, _)
    std::list<Statement*> expected_follows_3 = std::list<Statement*>{print_y_};
...
TEST_CASE("1.FollowsTExtractor.Extract FollowsT mixed containers") {
  ...
  SECTION("If and while") {
    ...
    deliverable.AddFollowRelationship(if_1_, while_1_);
    deliverable.AddFollowRelationship(assign_1_, assign_2_);
