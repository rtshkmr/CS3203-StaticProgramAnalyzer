...
bool Cluster::CheckScopeClusterForAffects(Cluster* scoped_cluster,
  ...
  if (target_range.first >= target_range.second) {
    ...
    while (!found) {
      ...
      if (first_stmt_to_while_end_is_unmodified && while_start_to_second_stmt_is_unmodified) {
        found = true;
        break;
      } else {
        current_scope = current_scope->GetParentCluster();
    ...
    if (tag == ClusterTag::kIfCluster) {
      ...
      if (is_target_in_if_cluster) {
        ...
        if (target_range.first < if_body_range.first) {
          ...
          bool else_body_is_unmodified_path = TraverseScopedClusterForAffects(else_body,
                                                                              else_body_range,
                                                                              pkb,
                                                                              lhs_var,
                                                                              goal_range);
