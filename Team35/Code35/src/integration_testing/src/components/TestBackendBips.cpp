...
TEST_CASE("NextBipExtractor basic integration") {
  ...
  SECTION("no call") {
    ...
    CHECK(AreEntityListsEqual(expected_next_lists[0], entities1));
    std::vector<Entity*> entities3 = next_bip_extractor.GetNextBip("3");
    CHECK(AreEntityListsEqual(expected_next_lists[2], entities3));
...
TEST_CASE("NextBipExtractor more procedures") {
  ...
  SECTION("call") {
    ...
    for (int i = 0; i < expected_next_bip_lists.size(); ++i) {
      ...
      std::vector<Entity*> entities = next_bip_extractor.GetNextBip(stmt_num);
